...
async function handlePaymentSuccess(db, event) {
  ...
  if (!bookingId || !userId) {
    console.error("Missing bookingId or userId in payment success webhook metadata");
    throw new Error("Missing required metadata in webhook");
  }

  console.log(`Processing payment success for booking: ${bookingId}, user: ${userId}`);
...
async function handlePaymentFailure(db, event) {
  ...
  if (!bookingId || !userId) {
    console.error("Missing bookingId or userId in payment failure webhook metadata");
    throw new Error("Missing required metadata in webhook");
  }

  console.log(`Processing payment failure for booking: ${bookingId}, user: ${userId}`);
  ...
  if (bookingDoc.exists) {
    await bookingRef.update({
      ...
      paymentError: checkoutSession.attributes.failure_reason || "Payment failed",
...
async function handleCheckoutExpired(db, event) {
  ...
  if (!bookingId || !userId) {
    console.error("Missing bookingId or userId in checkout expired webhook metadata");
    return; // Don't throw error for expired sessions
  }

  console.log(`Processing checkout expiry for booking: ${bookingId}, user: ${userId}`);
